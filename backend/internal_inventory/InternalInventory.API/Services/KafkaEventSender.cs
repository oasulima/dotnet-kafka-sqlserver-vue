...
namespace InternalInventory.API.Services
{
    public class KafkaEventSender : IEventSender
    {
        ...
        private string InternalInventoryItemReportingTopic => _options.Value.InternalInventoryItemReportingTopic;
        ...
        private void SendMessageWithRetries<TValue>(string topic, TValue messageValue,
            int retryCount = DefaultRetryCounts) where TValue : class
        {
            if (retryCount <= 0) return;

            KafkaUtils.Produce(_kafkaServers, topic, messageValue,
                dr =>
                {
                    if (dr.Error.IsError)
                    {
