...
public class LocatorQuoteResponseKafkaListener : BackgroundService
{
    ...
    private void ProcessMessage(ConsumeResult<string, QuoteResponse> consumeResult)
    {
        using var activity = TracingConfiguration.StartActivity(
            "LocatorQuoteResponseKafkaListener ProcessMessage"
        );
        try
        {
            var quoteResp = consumeResult.Message.Value;

            var quoteDetails = Converter.Deserialize<QuoteResponseDetails>(quoteResp.DetailsJson);
            ...
            if (quoteDetails?.Quote != null)
            {
                ...
                )
                {
                    foreach (var p in quote.ProviderQuoteRequests.Keys)
                    {
                        ...
                        _autoDisableProvidersService.RegisterProviderQuote(
                            p,
                            quote.Symbol,
                            isAnswered
                        );
                ...
                )
                {
                    foreach (
                        var p in quote
                            .ProviderBuyOrderRequests.Where(x =>
                                x.Value.request.ValidTill > DateTime.MinValue
                            )
                            .Select(x => x.Key)
                    )
                    {
